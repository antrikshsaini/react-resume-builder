...
const Template2 = ({ resume, renderPreview }) => {
  ...
  React.useEffect(() => {
    console.log("rendering preview in Template 1");
    renderPreview();
  }, []);

  if (!resume) {
    return <div>Loading or No Resume Data</div>; //handle no resume data.
  }
        ...
        <AppBar position="static">
          <Toolbar>
            <Typography variant="h5" className={classes.title}>
              <Link extact to="/builder" className={classes.link}>
                Go Back
              </Link>
            </Typography>
          </Toolbar>
            ...
            <div className="top-content row">
              ...
              <div className="contact-details flex-column col-sm">
                <div className="row ml-1 ">
                  ...
                  <span className="ml-3">{resume.personal.website}</span>
                </div>
                <div className="row ml-1">
                  <FontAwesomeIcon icon={faEnvelope} />
                  <span className="ml-3">{resume.personal.email}</span>
                </div>
                <div className="row ml-1">
                  <FontAwesomeIcon icon={faPhone} />{" "}
                  <span className="ml-3">{resume.personal.phone}</span>
                </div>
              </div>
            </div>
                      ...
                      <div className="details">
                        ...
                        <div>
                          {project.keywords.map((keyword, i) => (
                            <p key={i} className="inline keywords">
                              {keyword}{" "}
                            </p>
                          ))}
                        </div>
                      ...
                      <div className="details">
                        ...
                        <div>
                          {skill.keywords.map((keyword, i) => (
                            <p key={`index_${i}`} className="inline keywords">
                              {keyword}{" "}
                            </p>
                          ))}
                        </div>
